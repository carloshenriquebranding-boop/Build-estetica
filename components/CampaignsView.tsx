...
const CampaignsView: React.FC<CampaignsViewProps> = ({ clients, stages, showBackButton, onBack }) => {
  ...
  const EmailCampaign: React.FC = () => (
    <div className="space-y-4">
      ...
      <div>
        ...
        <select id="email-recipient" className={INPUT_CLASSES + ' md:w-1/2'}>
      ...
      <div>
        ...
        <input type="text" id="email-subject" className={INPUT_CLASSES} placeholder="Assunto do seu email" />
      </div>
      <div>
     ...
     <div className="space-y-4">
      <h2 className="text-xl font-bold">Criar Disparo de WhatsApp</h2>
       <div>
        ...
        <input type="text" id="wa-name" className={INPUT_CLASSES} placeholder="Ex: Promoção Dia das Mães" />
      </div>
      <div>
        <label htmlFor="wa-recipient" className="block text-sm font-medium text-gray-700 dark:text-slate-300">Destinatários</label>
        <select id="wa-recipient" className={INPUT_CLASSES + ' md:w-1/2'}>
          <option>Todos os Clientes ({clients.length})</option>
          {stages.map(s => <option key={s.id}>Clientes em: {s.title}</option>)}
        </select>
      </div>
      <div>
        ...
        <textarea id="wa-message" rows={6} className={INPUT_CLASSES} defaultValue="Olá {{nome_cliente}}! Temos uma novidade especial para você..."></textarea>
  ...
  return (
    <div>
      <ViewHeader title="Campanhas" showBackButton={showBackButton} onBack={onBack} />
